...
import { PreviewMarginDetail } from "~/components/previews/preview-margin"
...
`.is-margin-(way)-(value)` `.is-margin-(way)-minus-(value)` 形式で使います。`(way)` を省略すると全方向、`(value)` を省略した場合は `md` と同等値の Margin 付与となります。
...
`.is-margin-(value)` では上下左右に Margin を付与します。
