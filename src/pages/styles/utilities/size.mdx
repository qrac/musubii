...
import { PreviewSizeDetail } from "~/components/previews/preview-size"
...
`.is-(mediaquery)-(value)` の形式で利用。`(mediaquery)` を省略すると常時反映、`(value)` を省略した場合は `md` と同等の値になります。サイズ変更における倍率は [Font](/configs/variables/font) の値を引き継いでいます。
