...
SCSS 変数 `$option-css-variables` は「[Option](/configs/variables/option)」を参照ください。
...
これで Alert の CSS は出力されません。各スタイルの SCSS 変数は「[Option](/configs/variables/option)」を参照ください。
