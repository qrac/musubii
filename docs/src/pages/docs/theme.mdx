...
`data-theme="dark"` はローカルストレージの値を確認して付与することが一般的です。ただし、ページのレンダリングよりも先に付与しないと FOUC と呼ばれる CSS の適応遅れによるフラッシュ現象がおきます。そこで以下のようなスクリプトを HTML の `body` 内に設置してレンダリング前に実行します。
